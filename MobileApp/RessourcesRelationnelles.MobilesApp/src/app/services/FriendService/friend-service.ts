...
import { FriendRelation } from '../../models/friend.model';
...
export class FriendService {
  ...
  public getFriends(userId: number): Observable<FriendRelation[]> {
    return this.httpClient.get<FriendRelation[]>(`${API_ENDPOINTS.FRIENDS}/user/${userId}`).pipe(
      catchError(this.handleError)
    );
  }

  public getSentRequests(userId: number): Observable<FriendRelation[]> {
    return this.httpClient.get<FriendRelation[]>(`${API_ENDPOINTS.FRIENDS}/sender/${userId}`).pipe(
      catchError(this.handleError)
    );
  }

  public getReceivedRequests(userId: number): Observable<FriendRelation[]> {
    return this.httpClient.get<FriendRelation[]>(`${API_ENDPOINTS.FRIENDS}/receiver/${userId}`).pipe(
      catchError(this.handleError)
    );
  }

  public getFriendsAndRequests(userId: number): Observable<FriendRelation[]> {
    return this.httpClient.get<FriendRelation[]>(`${API_ENDPOINTS.FRIENDS}/friends/${userId}`).pipe(
      catchError(this.handleError)
    );
  }

  public sendFriendRequest(relation: FriendRelation): Observable<FriendRelation> {
    return this.httpClient.post<FriendRelation>(API_ENDPOINTS.FRIENDS, relation).pipe(
      catchError(this.handleError)
    );
  }

  public updateFriendRequest(id: number, relation: FriendRelation): Observable<FriendRelation> {
    return this.httpClient.put<FriendRelation>(`${API_ENDPOINTS.FRIENDS}/${id}`, relation).pipe(
      catchError(this.handleError)
    );
  }

  public deleteFriendship(id: number): Observable<void> {
    return this.httpClient.delete<void>(`${API_ENDPOINTS.FRIENDS}/${id}`).pipe(
      catchError(this.handleError)
    );
  }

  private handleError(error: any) {
    console.error('Erreur FriendService:', error);
    return throwError(() => new Error(error.message || 'Une erreur est survenue dans le service des relations d\'amitié.'));
  }
}
